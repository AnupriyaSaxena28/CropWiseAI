...
import { LANGUAGE_OPTIONS, type LanguageCode } from "@/app/chat/types";
  ...
  useEffect(() => {
    ...
    getUserProfile(user.uid).then((p) => {
      if (p) {
        ...
        if (p.preferredLanguage) setLang(p.preferredLanguage as LanguageCode);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
          ...
          <button onClick={handleSave} disabled={saving}
            ...
            {saved ? t("Saved!") : t("Update Profile")}
          </button>
        </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-20 gap-3">
            ...
            <p className="text-sm text-[#5a7460]">{t("Loading your profile...")}</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6">
            ...
            <div className="rounded-xl bg-[#111d16] border border-[#2a3d2c] p-6 space-y-5">
              <div className="flex items-center gap-2 pb-1 border-b border-[#1e2d20]">
                ...
                <h2 className="text-base font-bold text-[#e8f5e9]">{t("General Information")}</h2>
              </div>
              ...
              {user?.displayName && (
                <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#2ea82e]/5 border border-[#2ea82e]/20 text-xs text-[#4dc24d]">
                  ...
                  {t("Signed in as")} <span className="font-semibold">{user.email}</span> {t("via Google")}
                </div>
              )}

              {/* Farm Name */}
              <div>
                <label className="block text-xs font-medium text-[#5a7460] mb-1.5">{t("Farm Name")}</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-[#5a7460] mb-1.5">{t("Size (Acres)")}</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-[#5a7460] mb-1.5">{t("Soil Type")}</label>
              ...
              {/* Primary Crops */}
              <div>
                <label className="block text-xs font-medium text-[#5a7460] mb-2">{t("Primary Crops")}</label>
                <div className="flex flex-wrap gap-2 items-center">
                  ...
                  <div className="relative">
                    <button onClick={() => setShowCropPicker((p: boolean) => !p)}
                      ...
                      <Plus className="w-3 h-3" strokeWidth={2.5} /> {t("Add Crop")}
                    </button>
                    {showCropPicker && (
              ...
              {/* Preferred Language */}
              <div>
                <label className="block text-xs font-medium text-[#5a7460] mb-1.5">{t("Preferred Language")}</label>
                <select value={language}
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) => {
                    const v = e.target.value;
                    setLanguage(v);
                    setLang(v as LanguageCode); // apply across the whole app immediately
                  }}
                  className="input-field text-sm">
                  {LANGUAGES.map(l => <option key={l.code} value={l.code}>{l.label}</option>)}
                </select>
                <p className="text-[10px] text-[#5a7460] mt-1.5">
                  {t("Changes the language across the whole app and all AI responses.")}
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] text-[#5a7460] mb-1">{t("State")}</label>
                    <select value={state}
                  ...
                  <div>
                    <label className="block text-[10px] text-[#5a7460] mb-1">{t("District")}</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] text-[#5a7460] mb-1">{t("Pincode")}</label>
                  ...
                  <div>
                    <label className="block text-[10px] text-[#5a7460] mb-1">{t("GPS Coordinates")}</label>
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-xl bg-[#111d16] border border-[#2a3d2c] p-6 flex flex-col items-center">
                <h2 className="text-sm font-semibold text-[#e8f5e9] mb-1 self-start">
                  {t("Farm Intelligence Score")}
                </h2>
                <p className="text-xs text-[#5a7460] self-start mb-5">
                  {t("Based on profile completeness")}
                </p>
                ...
                {score < 100 && (
                  <div className="w-full mt-4 space-y-1.5">
                    ...
                    {!farmName   && <p className="text-xs text-[#5a7460]">• {t("Add farm name (+15)")}</p>}
                    {!landSize   && <p className="text-xs text-[#5a7460]">• {t("Add land size (+15)")}</p>}
                    {!state      && <p className="text-xs text-[#5a7460]">• {t("Add your state (+15)")}</p>}
                    {!district   && <p className="text-xs text-[#5a7460]">• {t("Add district (+10)")}</p>}
                    {crops.length === 0 && <p className="text-xs text-[#5a7460]">• {t("Add crops (+20)")}</p>}
                  </div>
                )}
              </div>
              ...
              <div className="rounded-xl bg-[#111d16] border border-[#2a3d2c] p-5 space-y-3">
                <h2 className="text-sm font-semibold text-[#e8f5e9]">{t("Account")}</h2>
                <div className="space-y-2">
                  {[
                    { label: t("Email"),        value: user?.email ?? "—"                        },
                    { label: t("Name"),         value: user?.displayName ?? t("Not set")         },
                    { label: t("Member since"), value: user?.metadata.creationTime
                        ? new Date(user.metadata.creationTime).toLocaleDateString("en-IN")
                        : "—"
                    },
                    { label: t("Last sign in"), value: user?.metadata.lastSignInTime
              ...
              <div className="flex items-start gap-3 p-3.5 rounded-xl bg-[#182419] border border-[#2a3d2c]">
                ...
                <div>
                  <p className="text-xs font-bold text-[#e8f5e9] mb-0.5">{t("Data Secured")}</p>
                  <p className="text-[11px] text-[#5a7460] leading-relaxed">
                    {t("Your farm data is encrypted and stored securely in Firebase. Only you can access it.")}
                  </p>
                </div>
              </div>
            </div>
