          ...
          <>
            ...
            {expanded && (
              <div className="space-y-4 pt-1 border-t border-[#1e2d20]">
                ...
                {/* Neo4j Sustainable Alternatives */}
                {result.treatment.sustainableAlternativesFromGraph && 
                 result.treatment.sustainableAlternativesFromGraph.length > 0 && (
                  <div className="mt-3 p-3 rounded-lg border border-[#2ea82e]/30 bg-[#1a4d1a]/10">
                    <p className="text-[10px] font-semibold uppercase tracking-widest text-[#4dc24d] mb-2 flex items-center gap-1.5">
                      <Sprout className="w-3 h-3" /> Eco-Friendly Alternatives (Knowledge Graph)
                    </p>
                    <div className="space-y-2">
                      {result.treatment.sustainableAlternativesFromGraph.map((alt, i) => (
                        <div key={i} className="flex flex-col gap-1">
                          <p className="text-xs font-semibold text-[#82d882]">{alt.name} <span className="text-[9px] text-[#5a7460] font-normal ml-1 border border-[#2a3d2c] rounded px-1">{alt.type}</span></p>
                          <p className="text-xs text-[#94a896] leading-relaxed">{alt.description}</p>
                        </div>
                      ))}
                    </div>
                  </div>
